...
test('should render error for invalid form', () =>{
    ...
    wrapper.find('form').simulate('submit',{
        preventDefault: () => {} //als we dit niet doen krijgen we een TypeError preventDefault
    });
...
test('should set description on input chage',()=>{
    ...
    wrapper.find('input').at(0).simulate('change',{
        target: { value }
    }); //we hebben meerdere inputs, en we hebben de at(eerste) nodig
...
test('should set note on input change', ()=>{
    ...
    expect(wrapper.state('note')).toBe('This is a note');
})
...
test('should call onSubmit prop for valid form',()=>{
    const onSubmitSpy = jest.fn(); //create fake functies
...
//volgende zijn in singledatepicker!
...
test('should set focus on focus change', ()=>{
    ...
    expect(wrapper.state('focus')).toBe(focus);
